...
const ViewPendingBags = (props) => {
    const AcceptHandler = async () => {
        try {
            const response = await fetch('https://onlinebloodbank.azurewebsites.net/BloodBag/AcceptBloodBag/' + props.BloodBag._id, {
              method: 'PUT',
              headers: {
    ...
    const RejectHandler = async () => {
        try {
            const response = await fetch('https://onlinebloodbank.azurewebsites.net/BloodBag/RemoveBloodBag/' + props.BloodBag._id, {
              method: 'DELETE',
              headers: {
                'Content-Type': 'application/json'
                //Authorization: `BEARER ${authContext.token}`
